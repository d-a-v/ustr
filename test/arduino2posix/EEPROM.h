...
public:
    ...
    inline static void put(int x, char c)
    {
        if (x > USTR_SIZE_EE)
        {
            fprintf(stderr, "### eeprom beyond\r\n");
        }
        else if (pwrite(fd, &c, 1, x) != 1)
        {
            fprintf(stderr, "eeprom: %s\n\r", strerror(errno));
        }
    }
    inline static void update(int x, char c)
    {
        put(x, c);
    }
} EEPROM;
